  ...
  fluidPage(
    fluidRow(
      ...
      box(
        ...
        input_switch(ns("switch"), "Plot Updates"),
        plotOutput(ns("plot")) %>%
    ...
    fluidRow(
      box(
        title = "Data", collapsible = TRUE, solidHeader = TRUE, status = "primary", width = 6, collapsed = FALSE,
        rHandsontableOutput(ns("hot"))
      ),
      box(
        title = "Colour Key", collapsible = TRUE, solidHeader = TRUE, status = "primary", width = 6, collapsed = FALSE,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$upload_layout_table, {
        ...
        updateSelectizeInput(session, "displayed_genes", choices = unique(session$userData$vars$qPCR_layout$Gene),
      ...
      output$plot <- renderImage(
        {
          ...
          df <- full_join(session$userData$vars$qPCR_layout,
                          session$userData$vars$Cq_table,by = "Well", suffix = c("", ".Cq")) |>
            janitor::clean_names()%>%
            left_join(.,{. |> 
            ...
            left_join(.,{. |> 
                ...
                summarise(control_cq = mean(diff_cq),
                          .by = c(gene))}) |> 
            mutate(expression = 2^(diff_cq),,

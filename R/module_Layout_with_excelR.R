UI_Layout <- function(id) {
  ...
  fluidPage(fluidRow(
    tabBox(
      ...
      tabPanel("Gene", excelOutput(ns("hot_Gene"))),
      tabPanel("Sample", excelOutput(ns("hot_Sample"))),
      tabPanel("Sample Key", excelOutput(ns("hot_sample_key")))
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$hot_Gene, {
      session$userData$vars$layout <- session$userData$vars$layout |> 
        select(-Gene) |>
        left_join({excel_to_R(input$hot_Gene) |>
    ...
    observeEvent(input$hot_Sample, {
      session$userData$vars$layout <- session$userData$vars$layout |> 
        select(-Sample) |>
        left_join({excel_to_R(input$hot_Sample) |>
            pivot_longer(everything(),
                         names_to = "Col",
                         values_to = "Sample") |>
            mutate(Well = wellr::well_from_index(1:as.numeric(input$plate_size), plate = as.numeric(input$plate_size), num_width = 0)) |>
    ...
    observeEvent(input$hot_sample_key, {
      session$userData$vars$layout <- session$userData$vars$layout |> 
        ...
        left_join(excel_to_R(input$hot_sample_key))
    })
    
    # Prepare Hot tables for Gene Sample and Treatment--------------------------
    

    output$hot_Gene = renderExcel({
      excelTable({
        session$userData$vars$layout |>
          select(Well, Gene) |>
          mutate(
    ...
    output$hot_Sample = renderExcel({
      excelTable({
        df <- session$userData$vars$layout |>
          select(Well, Sample) |>
          mutate(
            Col = wellr::well_to_col_num(Well),
            Row = wellr::well_to_row_let(Well),
    ...
    output$hot_sample_key = renderExcel({
      excelTable({
        session$userData$vars$layout |>
